...
const GallerySection = ({ creativeMode }: GallerySectionProps) => {
  ...
  const goNext = () => {
    if (lightboxIndex !== null) setLightboxIndex((lightboxIndex + 1) % paintings.length);
  };
  const goPrev = () => {
    if (lightboxIndex !== null) setLightboxIndex((lightboxIndex - 1 + paintings.length) % paintings.length);
  };
  ...
  return (
    <section id="gallery" className="py-24 px-6">
      ...
      {lightboxIndex !== null && (
        ...
        >
          <button onClick={closeLightbox} className="absolute top-6 right-6 text-muted-foreground hover:text-foreground transition-colors z-10">
            <X className="w-6 h-6" />
          </button>
          <button onClick={(e) => { e.stopPropagation(); goPrev(); }} className="absolute left-4 sm:left-8 text-muted-foreground hover:text-foreground transition-colors z-10">
            <ChevronLeft className="w-8 h-8" />
          </button>
          <div className="max-w-4xl max-h-[80vh] px-12" onClick={(e) => e.stopPropagation()}>
            <img src={paintings[lightboxIndex].full} alt={paintings[lightboxIndex].title} className="max-w-full max-h-[75vh] object-contain rounded-lg shadow-2xl" />
            <p className="text-center mt-4 font-heading font-semibold text-foreground">{paintings[lightboxIndex].title}</p>
          </div>
          <button onClick={(e) => { e.stopPropagation(); goNext(); }} className="absolute right-4 sm:right-8 text-muted-foreground hover:text-foreground transition-colors z-10">
